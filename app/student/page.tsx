...
export default function StudentPage() {
  ...
  const connect = useCallback(() => {
    ...
    ws.onclose = () => {
      ...
      if (!lessonEndedRef.current && (stateRef.current === 'active' || stateRef.current === 'connecting')) {
        const m = stateRef.current === 'connecting' ? 'Не удалось подключиться к серверу' : 'Соединение прервано'
        setStateSynced('error')
        setErrorMsg(m)
      }
    }
  ...
  return (
    <main className="min-h-screen flex flex-col bg-gray-950">
      ...
      {/* Subtitles — extra bottom padding to clear the fixed button panel */}
      <div className="flex-1 overflow-y-auto px-5 py-8 space-y-6 pb-28">
      ...
      {/* Fixed signal buttons panel */}
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-gray-900/95 backdrop-blur-sm border-t border-gray-800 px-2 py-2">
